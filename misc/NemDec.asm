...
NemDec:
		...
		lea     NemDec_WriteAndStay(pc),a3 ;  8 a3  = jump address for pixel row writes
		lea     (VDP_data_port).l,a4                  ; 12 a4 -> [vdp_data_port]
NemDec_Main:
		; INPUT:
		;   a3  = NemDec_WriteRowToRAM/VDP
		;   a4 -> [$C00000] (vdp_data_port) (or anywhere in RAM)

		lea     (Decomp_Buffer).w,a1          ;  8 a1 -> [$FFAA00] (Nemesis decompression buffer)

		move.w  (a0)+,d3                ;  8 d3 <- #/patterns (MSB set for Mode 1)
		bpl.s   +                      ; 10/8 d3.15 == 1 ? no, so in Mode 0 (not based on changes between rows)

		lea     NemDec_WriteAndStay_XOR-NemDec_WriteAndStay(a3),a3 ;  8 yes, so in Mode 1 (each row XOR'd with last, only changes recorded)
+
		lsl.w   #3,d3                   ; 6 + 2*3 = 12 -.
		subq.w  #1,d3                   ;            4 -' d3 = #/patterns * 8 (= number of rows to plot), minus 1

		bsr.w   NemDecPrepare                 ; 18 d0-d2/d5/a5 build opcode-to-count/color lookup table
...
; ---------------------------------------------------------------------------
; NemDec_WriteIter
NemDec_WriteIter: ; this is an entry point for compatibility with code in sonic.asm
...
NemDec2:
		...
		bcc.s   +                      ; no,  so not enough room to read next byte
...
NemDec_NotInline:
		cmpi.b  #9,d6                   ; 9 or more bits still available ?
		bcc.s   +                      ; no,  so not enough room for read next byte
...
loc_1502:
		...
		dbra    d3,NemDec_WriteIter_1            ; 10/12 have all 8-pixel rows been written ? if yes, branch
